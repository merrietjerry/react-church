...
import { API_BASE_URL, API_AUTH_BASE_URL } from '../constants/api'
...
export function callApi(endpoint, method, data) {
  ...
  return axios(baseURL + endpoint, config).then(res => {
    let response = res
...
export default store => next => action => {
  ...
  function actionWith(data) {
    ...
    if(finalAction.type === 'LOGIN_SUCCESS'){
      if(finalAction.response)
        localStorage.setItem('user', finalAction.response.data.phone)
    }
